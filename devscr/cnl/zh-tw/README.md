...
devSCR-CNL 定義了一種 **受控規範語言 (Controlled Normative Language)**，用於向 AI 系統表達 **強制性的行為約束**。
...
devSCR-CNL **僅表達可執行的行為約束**。

---

## 2. 適用範圍（Scope）

devSCR-CNL 適用於**任何必須約束 AI 行為的交互場景**。
...
devSCR-CNL 將規範語態（Normative Modality）限制在一個極小且明確的集合內：
...
devSCR-CNL 遵循一種受限的語句結構，旨在減少歧義，同時保留自然語言的可讀性。
...
devSCR-CNL 強烈偏好**以產出物為導向的主體 (Artifact-oriented subjects)**，而非對話式或基於角色的主體。
...
devSCR-CNL 始終是一項**語言紀律 (Language Discipline)**，而非程式語言。

---

## 6. 範例 (Examples)

本節提供 devSCR-CNL 用法之**說明性範例**。
...
devSCR-CNL 的 schema **不是為了寫程式邏輯**，而是為了：
